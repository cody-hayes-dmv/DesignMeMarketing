import React, { useState, useEffect, useMemo, useRef } from "react";
import { X, Plus, Clock, User, AlertCircle, CheckCircle, ListChecks, ChevronDown, ChevronRight } from "lucide-react";
import toast from "react-hot-toast";
import api from "@/lib/api";

interface OnboardingTemplate {
  id: string;
  name: string;
  description: string | null;
  isDefault: boolean;
  tasks: OnboardingTask[];
}

interface OnboardingTask {
  id: string;
  title: string;
  description: string | null;
  category: string | null;
  priority: string | null;
  estimatedHours: number | null;
  dueDate: string | null;
  dueDaysAfterStart?: number | null;
  order: number;
}

interface Client {
  id: string;
  name: string;
  domain: string;
}

interface User {
  id: string;
  name: string;
  email: string;
  role?: string;
}

interface OnboardingTemplateModalProps {
  open: boolean;
  setOpen: (open: boolean) => void;
  onTasksCreated: () => void;
  /** When opening from a client context (e.g. Client Dashboard), preselect this client */
  initialClientId?: string;
}

const OnboardingTemplateModal: React.FC<OnboardingTemplateModalProps> = ({
  open,
  setOpen,
  onTasksCreated,
  initialClientId,
}) => {
  const [templates, setTemplates] = useState<OnboardingTemplate[]>([]);
  const [clients, setClients] = useState<Client[]>([]);
  const [templateAssignableUsers, setTemplateAssignableUsers] = useState<User[]>([]);
  const [templateAssignableLoading, setTemplateAssignableLoading] = useState(false);
  const [templateAssignableSearch, setTemplateAssignableSearch] = useState("");
  const [templateAssignToOpen, setTemplateAssignToOpen] = useState(false);
  const [templateAssigneeDisplay, setTemplateAssigneeDisplay] = useState("");
  const templateAssignToRef = useRef<HTMLDivElement>(null);
  const [deliverableAssignableUsers, setDeliverableAssignableUsers] = useState<User[]>([]);
  const [deliverableAssignableLoading, setDeliverableAssignableLoading] = useState(false);
  const [deliverableAssignableSearch, setDeliverableAssignableSearch] = useState("");
  const [deliverableAssignToOpen, setDeliverableAssignToOpen] = useState(false);
  const [deliverableAssigneeDisplay, setDeliverableAssigneeDisplay] = useState("");
  const deliverableAssignToRef = useRef<HTMLDivElement>(null);
  const [selectedTemplate, setSelectedTemplate] = useState<OnboardingTemplate | null>(null);
  const [selectedClient, setSelectedClient] = useState<string>(initialClientId || "");
  const [selectedTemplateSpecialist, setSelectedTemplateSpecialist] = useState<string>("");
  const [selectedDeliverableSpecialist, setSelectedDeliverableSpecialist] = useState<string>("");
  const [selectedTemplateTaskIds, setSelectedTemplateTaskIds] = useState<string[]>([]);
  const [selectedDeliverableTaskIds, setSelectedDeliverableTaskIds] = useState<string[]>([]);
  const [templateTasksExpanded, setTemplateTasksExpanded] = useState(true);
  const [deliverablesExpanded, setDeliverablesExpanded] = useState(true);
  const [onboardingStartDate, setOnboardingStartDate] = useState<Date>(() => new Date());
  const [autoGenerateDeliverables, setAutoGenerateDeliverables] = useState(true);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string>("");

  const DELIVERABLES_START_DAY_CUTOFF = 20;

  const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

  const formatYYYYMMDD = (d: Date) => {
    const y = d.getFullYear();
    const m = String(d.getMonth() + 1).padStart(2, "0");
    const day = String(d.getDate()).padStart(2, "0");
    return `${y}-${m}-${day}`;
  };

  const onboardingStartDateStr = formatYYYYMMDD(onboardingStartDate);

  const parseYYYYMMDD = (value: string): Date => {
    // Parse as local date to avoid timezone shifts.
    const [y, m, d] = value.split("-").map((p) => Number(p));
    return new Date(y, (m || 1) - 1, d || 1);
  };

  const addDaysToLocalDate = (base: Date, days: number): Date => {
    const d = new Date(base);
    d.setHours(0, 0, 0, 0);
    d.setDate(d.getDate() + days);
    return d;
  };

  const getDeliverablesStartMonth = (start: Date) => {
    const d = new Date(start);
    d.setHours(0, 0, 0, 0);
    // If start date is before the 20th, current month counts; otherwise start next month.
    if (d.getDate() < DELIVERABLES_START_DAY_CUTOFF) {
      return new Date(d.getFullYear(), d.getMonth(), 1);
    }
    return new Date(d.getFullYear(), d.getMonth() + 1, 1);
  };

  const endOfMonth = (monthStart: Date) => {
    const d = new Date(monthStart.getFullYear(), monthStart.getMonth() + 1, 0);
    d.setHours(0, 0, 0, 0);
    return d;
  };

  const buildMonthlyDeliverablesTaskDrafts = () => {
    const firstMonth = getDeliverablesStartMonth(onboardingStartDate);
    return Array.from({ length: 12 }).map((_, idx) => {
      const monthStart = new Date(firstMonth.getFullYear(), firstMonth.getMonth() + idx, 1);
      const monthLabel = `${monthNames[monthStart.getMonth()]} ${monthStart.getFullYear()}`;
      return {
        tempId: `deliverable-${monthStart.getFullYear()}-${String(monthStart.getMonth() + 1).padStart(2, "0")}`,
        title: `Monthly Deliverables - ${monthLabel}`,
        description: `Monthly deliverables for ${monthLabel}.`,
        category: "Deliverables",
        dueDate: formatYYYYMMDD(endOfMonth(monthStart)),
        priority: "medium",
      };
    });
  };

  const deliverablesPreviewTasks = useMemo(
    () => (autoGenerateDeliverables ? buildMonthlyDeliverablesTaskDrafts() : []),
    [autoGenerateDeliverables, onboardingStartDate]
  );

  const buildMonthlyDeliverablesTasks = () => {
    return deliverablesPreviewTasks.map((draftTask) => ({
      title: draftTask.title,
      description: draftTask.description,
      category: draftTask.category,
      status: "TODO" as const,
      clientId: selectedClient,
      assigneeId: selectedDeliverableSpecialist || null,
      dueDate: draftTask.dueDate,
      priority: draftTask.priority,
    }));
  };

  const getDeliverablesRangeLabel = () => {
    const first = getDeliverablesStartMonth(onboardingStartDate);
    const last = new Date(first.getFullYear(), first.getMonth() + 11, 1);
    return `${monthNames[first.getMonth()]} ${first.getFullYear()} – ${monthNames[last.getMonth()]} ${last.getFullYear()}`;
  };

  // Fetch templates, clients, and specialists on modal open
  useEffect(() => {
    if (open) {
      setSelectedClient(initialClientId || "");
      setOnboardingStartDate(new Date());
      fetchTemplates();
      fetchClients();
    }
  }, [open, initialClientId]);

  // Preselect client once clients list is loaded (e.g. when opened from Client Dashboard)
  useEffect(() => {
    if (open && initialClientId && clients.length > 0 && clients.some((c) => c.id === initialClientId)) {
      setSelectedClient(initialClientId);
    }
  }, [open, initialClientId, clients]);

  useEffect(() => {
    if (!selectedTemplate) {
      setSelectedTemplateTaskIds([]);
      return;
    }
    setSelectedTemplateTaskIds((prev) => prev.filter((id) => selectedTemplate.tasks.some((t) => t.id === id)));
  }, [selectedTemplate]);

  useEffect(() => {
    if (!autoGenerateDeliverables) {
      setSelectedDeliverableTaskIds([]);
      return;
    }
    const validIds = new Set(deliverablesPreviewTasks.map((task) => task.tempId));
    setSelectedDeliverableTaskIds((prev) => prev.filter((id) => validIds.has(id)));
  }, [autoGenerateDeliverables, deliverablesPreviewTasks]);

  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    setTemplateAssignableLoading(true);
    api
      .get("/tasks/assignable-users", {
        params: templateAssignableSearch.trim() ? { search: templateAssignableSearch.trim() } : {},
      })
      .then((res) => {
        if (!cancelled && Array.isArray(res.data)) setTemplateAssignableUsers(res.data);
      })
      .catch(() => {
        if (!cancelled) setTemplateAssignableUsers([]);
      })
      .finally(() => {
        if (!cancelled) setTemplateAssignableLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [open, templateAssignableSearch]);

  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    setDeliverableAssignableLoading(true);
    api
      .get("/tasks/assignable-users", {
        params: deliverableAssignableSearch.trim() ? { search: deliverableAssignableSearch.trim() } : {},
      })
      .then((res) => {
        if (!cancelled && Array.isArray(res.data)) setDeliverableAssignableUsers(res.data);
      })
      .catch(() => {
        if (!cancelled) setDeliverableAssignableUsers([]);
      })
      .finally(() => {
        if (!cancelled) setDeliverableAssignableLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [open, deliverableAssignableSearch]);

  useEffect(() => {
    if (!templateAssignToOpen && !deliverableAssignToOpen) return;
    const onDocClick = (e: MouseEvent) => {
      const targetNode = e.target as Node;
      if (templateAssignToRef.current && !templateAssignToRef.current.contains(targetNode)) {
        setTemplateAssignToOpen(false);
      }
      if (deliverableAssignToRef.current && !deliverableAssignToRef.current.contains(targetNode)) {
        setDeliverableAssignToOpen(false);
      }
    };
    document.addEventListener("click", onDocClick);
    return () => document.removeEventListener("click", onDocClick);
  }, [templateAssignToOpen, deliverableAssignToOpen]);

  const fetchTemplates = async () => {
    try {
      const response = await api.get("/onboarding/templates");
      const data = response.data;
      setTemplates(Array.isArray(data) ? data : Array.isArray(data?.templates) ? data.templates : []);
    } catch (error: any) {
      console.error("Error fetching templates:", error);
      setTemplates([]);
      // Toast is already shown by API interceptor
    }
  };

  const fetchClients = async () => {
    try {
      const response = await api.get("/clients");
      const list = Array.isArray(response.data) ? response.data : [];
      list.sort((a: Client, b: Client) =>
        (a.name || "").localeCompare(b.name || "", undefined, { sensitivity: "base" })
      );
      setClients(list);
    } catch (error: any) {
      console.error("Error fetching clients:", error);
      // Toast is already shown by API interceptor
    }
  };

  const handleCreateTasks = async () => {
    if (!selectedTemplate || !selectedClient) {
      setError("Please select a template and client");
      return;
    }

    setLoading(true);
    setError("");

    try {
      const tasks = selectedTemplate.tasks.map((templateTask) => ({
        dueDate:
          templateTask.dueDaysAfterStart != null
            ? formatYYYYMMDD(addDaysToLocalDate(onboardingStartDate, Math.max(0, Number(templateTask.dueDaysAfterStart))))
            : templateTask.dueDate
            ? templateTask.dueDate.slice(0, 10)
            : null,
        title: templateTask.title,
        description: templateTask.description ?? undefined,
        category: templateTask.category ?? undefined,
        status: "TODO" as const,
        clientId: selectedClient,
        assigneeId: selectedTemplateSpecialist || null,
        estimatedHours: templateTask.estimatedHours ?? undefined,
        priority: templateTask.priority ?? undefined,
      }));

      const deliverablesTasks = autoGenerateDeliverables ? buildMonthlyDeliverablesTasks() : [];

      const allTasks = [...tasks, ...deliverablesTasks];

      await api.post("/tasks/bulk", { tasks: allTasks });
      
      toast.success(
        `Successfully created ${allTasks.length} task${allTasks.length > 1 ? "s" : ""}!`
      );
      onTasksCreated();
      setOpen(false);
      resetForm();
    } catch (error: any) {
      const errorMsg = error.response?.data?.message || "Failed to create tasks";
      setError(errorMsg);
      // Toast is already shown by API interceptor
    } finally {
      setLoading(false);
    }
  };

  const resetForm = () => {
    setSelectedTemplate(null);
    setSelectedClient("");
    setSelectedTemplateSpecialist("");
    setSelectedDeliverableSpecialist("");
    setTemplateAssignableSearch("");
    setDeliverableAssignableSearch("");
    setTemplateAssigneeDisplay("");
    setDeliverableAssigneeDisplay("");
    setTemplateAssignToOpen(false);
    setDeliverableAssignToOpen(false);
    setSelectedTemplateTaskIds([]);
    setSelectedDeliverableTaskIds([]);
    setTemplateTasksExpanded(true);
    setDeliverablesExpanded(true);
    setOnboardingStartDate(new Date());
    setAutoGenerateDeliverables(true);
    setError("");
  };

  const toggleTemplateTaskSelection = (taskId: string, checked: boolean) => {
    setSelectedTemplateTaskIds((prev) => {
      if (checked) return prev.includes(taskId) ? prev : [...prev, taskId];
      return prev.filter((id) => id !== taskId);
    });
  };

  const toggleSelectAllTemplateTasks = (checked: boolean) => {
    if (!selectedTemplate) return;
    setSelectedTemplateTaskIds(checked ? selectedTemplate.tasks.map((t) => t.id) : []);
  };

  const clearTemplateTaskSelection = () => {
    setSelectedTemplateTaskIds([]);
  };

  const toggleDeliverableTaskSelection = (taskId: string, checked: boolean) => {
    setSelectedDeliverableTaskIds((prev) => {
      if (checked) return prev.includes(taskId) ? prev : [...prev, taskId];
      return prev.filter((id) => id !== taskId);
    });
  };

  const toggleSelectAllDeliverableTasks = (checked: boolean) => {
    setSelectedDeliverableTaskIds(checked ? deliverablesPreviewTasks.map((t) => t.tempId) : []);
  };

  const clearDeliverableTaskSelection = () => {
    setSelectedDeliverableTaskIds([]);
  };

  const getPriorityColor = (priority: string | null) => {
    switch (priority) {
      case "high":
        return "text-red-600 bg-red-100";
      case "medium":
        return "text-yellow-600 bg-yellow-100";
      case "low":
        return "text-green-600 bg-green-100";
      default:
        return "text-gray-600 bg-gray-100";
    }
  };

  const getPriorityIcon = (priority: string | null) => {
    switch (priority) {
      case "high":
        return <AlertCircle className="h-3 w-3" />;
      case "medium":
        return <Clock className="h-3 w-3" />;
      case "low":
        return <CheckCircle className="h-3 w-3" />;
      default:
        return <Clock className="h-3 w-3" />;
    }
  };

  if (!open) return null;

  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl ring-1 ring-gray-200/80 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 flex-shrink-0 bg-gradient-to-r from-primary-600 via-primary-500 to-blue-600 text-white rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-white/20">
              <ListChecks className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-2xl font-bold">Use a template</h2>
              <p className="text-white/90 mt-0.5">Generate tasks from predefined templates for new clients</p>
            </div>
          </div>
          <button
            onClick={() => setOpen(false)}
            className="p-2 rounded-lg text-white/90 hover:bg-white/20 hover:text-white transition-colors"
          >
            <X className="h-6 w-6" />
          </button>
        </div>

        {/* Content */}
        <div className="p-6 overflow-y-auto flex-1 min-h-0 bg-gray-50/50">
          {error && (
            <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-center">
                <AlertCircle className="h-5 w-5 text-red-500 mr-2" />
                <span className="text-red-700">{error}</span>
              </div>
            </div>
          )}

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Template Selection */}
            <div className="rounded-xl border-l-4 border-blue-500 bg-blue-50/50 p-4 sm:p-5">
              <h3 className="text-sm font-semibold text-blue-900 mb-4 flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                Select Template
              </h3>
              <div className="space-y-3">
                {templates.length === 0 ? (
                  <p className="text-sm text-gray-500 py-4">No templates available. Loading or creating default…</p>
                ) : null}
                {templates.map((template) => (
                  <div
                    key={template.id}
                    onClick={() => setSelectedTemplate(template)}
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                      selectedTemplate?.id === template.id
                        ? "border-primary-500 bg-primary-50"
                        : "border-gray-200 hover:border-gray-300"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-medium text-gray-900">{template.name}</h4>
                        {template.description && (
                          <p className="text-sm text-gray-600 mt-1">{template.description}</p>
                        )}
                        <p className="text-xs text-gray-500 mt-1">
                          {template.tasks.length} tasks • {template.tasks.reduce((sum, task) => sum + (task.estimatedHours || 0), 0)} hours
                        </p>
                      </div>
                      {template.isDefault && (
                        <span className="px-2 py-1 text-xs font-medium text-primary-600 bg-primary-100 rounded-full">
                          Default
                        </span>
                      )}
                    </div>
                  </div>
                ))}
              </div>
            </div>

            {/* Configuration */}
            <div className="rounded-xl border-l-4 border-emerald-500 bg-emerald-50/50 p-4 sm:p-5">
              <h3 className="text-sm font-semibold text-emerald-900 mb-4 flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                Configuration
              </h3>
              <div className="space-y-4">
                {/* Client Selection */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Client *
                  </label>
                  <select
                    value={selectedClient}
                    onChange={(e) => setSelectedClient(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  >
                    <option value="">Select a client</option>
                    {clients.map((client) => (
                      <option key={client.id} value={client.id}>
                        {client.name} ({client.domain})
                      </option>
                    ))}
                  </select>
                </div>

                {/* Onboarding Start Date */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Onboarding start date
                  </label>
                  <input
                    type="date"
                    value={onboardingStartDateStr}
                    onChange={(e) => setOnboardingStartDate(parseYYYYMMDD(e.target.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  />
                  <p className="mt-1 text-xs text-gray-500">
                    If the start date is before day {DELIVERABLES_START_DAY_CUTOFF}, the current month is included in the 12 months.
                  </p>
                </div>

                {/* Auto-generate Deliverables */}
                <div className="flex items-start gap-3">
                  <input
                    id="autoGenerateDeliverables"
                    type="checkbox"
                    checked={autoGenerateDeliverables}
                    onChange={(e) => setAutoGenerateDeliverables(e.target.checked)}
                    className="mt-1 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                  />
                  <div className="flex-1">
                    <label htmlFor="autoGenerateDeliverables" className="text-sm font-medium text-gray-700">
                      Auto-generate 12 months of deliverables
                    </label>
                    <p className="text-xs text-gray-500">
                      Creates monthly deliverable tasks for the next 12 months.
                      {autoGenerateDeliverables ? ` (${getDeliverablesRangeLabel()})` : ""}
                    </p>
                  </div>
                </div>

              </div>
            </div>
          </div>

          {/* Selected Template Tasks Preview */}
          {selectedTemplate && (
            <div className="mt-6 rounded-xl border-l-4 border-amber-500 bg-amber-50/50 p-4 sm:p-5">
              <button
                type="button"
                onClick={() => setTemplateTasksExpanded((prev) => !prev)}
                className="w-full text-left"
              >
                <h3 className="text-sm font-semibold text-amber-900 mb-4 flex items-center justify-between gap-2">
                  <span className="inline-flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                    Tasks to be Created ({selectedTemplate.tasks.length})
                  </span>
                  {templateTasksExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </h3>
              </button>
              {templateTasksExpanded && (
              <>
              <div className="mb-3 rounded-lg border border-amber-200 bg-white px-3 py-2">
                <div className="flex flex-wrap items-center gap-2">
                  <label className="inline-flex items-center gap-2 text-sm text-gray-700">
                    <input
                      type="checkbox"
                      checked={
                        selectedTemplate.tasks.length > 0 &&
                        selectedTemplateTaskIds.length === selectedTemplate.tasks.length
                      }
                      onChange={(e) => toggleSelectAllTemplateTasks(e.target.checked)}
                      className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-2 focus:ring-primary-500"
                    />
                    Select all
                  </label>
                  <span className="text-sm font-medium text-gray-700">
                    Selected: {selectedTemplateTaskIds.length}
                  </span>
                  <div ref={templateAssignToRef} className="relative w-56 sm:w-64 shrink-0">
                    <input
                      type="text"
                      value={selectedTemplateSpecialist ? templateAssigneeDisplay : templateAssignableSearch}
                      onChange={(e) => {
                        setTemplateAssignableSearch(e.target.value);
                        setTemplateAssignToOpen(true);
                        setDeliverableAssignToOpen(false);
                        if (selectedTemplateSpecialist && e.target.value !== templateAssigneeDisplay) {
                          setSelectedTemplateSpecialist("");
                          setTemplateAssigneeDisplay("");
                        }
                      }}
                      onFocus={() => {
                        setTemplateAssignToOpen(true);
                        setDeliverableAssignToOpen(false);
                      }}
                      placeholder="Assigned selector"
                      className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                    {templateAssignToOpen && (
                      <ul className="absolute z-50 mt-1 w-full max-h-48 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg py-1">
                        {templateAssignableLoading ? (
                          <li className="px-3 py-2 text-sm text-gray-500">Loading…</li>
                        ) : templateAssignableUsers.length === 0 ? (
                          <li className="px-3 py-2 text-sm text-gray-500">No users found. Try a different search.</li>
                        ) : (
                          templateAssignableUsers.map((u) => (
                            <li key={u.id}>
                              <button
                                type="button"
                                className="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 flex items-center justify-between"
                                onClick={() => {
                                  setSelectedTemplateSpecialist(u.id);
                                  setTemplateAssigneeDisplay(u.name || u.email);
                                  setTemplateAssignableSearch("");
                                  setTemplateAssignToOpen(false);
                                }}
                              >
                                <span>{u.name || u.email}</span>
                                {u.role && <span className="text-xs text-gray-500 ml-2">{u.role.replace("_", " ")}</span>}
                              </button>
                            </li>
                          ))
                        )}
                      </ul>
                    )}
                  </div>
                  <button
                    type="button"
                    onClick={clearTemplateTaskSelection}
                    disabled={selectedTemplateTaskIds.length === 0}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Clear selection
                  </button>
                </div>
              </div>
              <div className="space-y-3">
                {selectedTemplate.tasks
                  .sort((a, b) => a.order - b.order)
                  .map((task, index) => (
                    <div key={task.id} className="p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-start">
                        <div className="flex-1">
                          <div className="flex items-center space-x-2">
                            <input
                              type="checkbox"
                              checked={selectedTemplateTaskIds.includes(task.id)}
                              onChange={(e) => toggleTemplateTaskSelection(task.id, e.target.checked)}
                              className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-2 focus:ring-primary-500"
                            />
                            <span className="text-sm font-medium text-gray-500">#{index + 1}</span>
                            <h4 className="font-medium text-gray-900">{task.title}</h4>
                            {task.priority && (
                              <span className={`px-2 py-1 text-xs font-medium rounded-full flex items-center space-x-1 ${getPriorityColor(task.priority)}`}>
                                {getPriorityIcon(task.priority)}
                                <span className="capitalize">{task.priority}</span>
                              </span>
                            )}
                          </div>
                          {task.description && (
                            <p className="text-sm text-gray-600 mt-1">{task.description}</p>
                          )}
                          <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                            {task.category && (
                              <span className="px-2 py-1 bg-gray-200 rounded">
                                {task.category}
                              </span>
                            )}
                            {task.dueDaysAfterStart != null ? (
                              <span className="px-2 py-1 bg-primary-100 text-primary-700 rounded">
                                Due: {task.dueDaysAfterStart} day{task.dueDaysAfterStart === 1 ? "" : "s"} after start
                              </span>
                            ) : task.dueDate ? (
                              <span className="px-2 py-1 bg-primary-100 text-primary-700 rounded">
                                Due: {task.dueDate.slice(0, 10)}
                              </span>
                            ) : (
                              <span className="px-2 py-1 bg-white border border-gray-200 rounded text-gray-500">
                                No due date
                              </span>
                            )}
                            {task.estimatedHours && (
                              <span className="flex items-center space-x-1">
                                <Clock className="h-3 w-3" />
                                <span>{task.estimatedHours}h</span>
                              </span>
                            )}
                          </div>
                        </div>
                      </div>
                    </div>
                  ))}
              </div>
              </>
              )}
            </div>
          )}

          {/* Auto-generated Deliverables Preview */}
          {autoGenerateDeliverables && (
            <div className="mt-6 rounded-xl border-l-4 border-violet-500 bg-violet-50/50 p-4 sm:p-5">
              <button
                type="button"
                onClick={() => setDeliverablesExpanded((prev) => !prev)}
                className="w-full text-left"
              >
                <h3 className="text-sm font-semibold text-violet-900 mb-4 flex items-center justify-between gap-2">
                  <span className="inline-flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-violet-500" />
                    Auto-generated Deliverables ({deliverablesPreviewTasks.length})
                  </span>
                  {deliverablesExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </h3>
              </button>
              {deliverablesExpanded && (
              <>
              <div className="mb-3 rounded-lg border border-violet-200 bg-white px-3 py-2">
                <div className="flex flex-wrap items-center gap-2">
                  <label className="inline-flex items-center gap-2 text-sm text-gray-700">
                    <input
                      type="checkbox"
                      checked={
                        deliverablesPreviewTasks.length > 0 &&
                        selectedDeliverableTaskIds.length === deliverablesPreviewTasks.length
                      }
                      onChange={(e) => toggleSelectAllDeliverableTasks(e.target.checked)}
                      className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-2 focus:ring-primary-500"
                    />
                    Select all
                  </label>
                  <span className="text-sm font-medium text-gray-700">
                    Selected: {selectedDeliverableTaskIds.length}
                  </span>
                  <div ref={deliverableAssignToRef} className="relative w-56 sm:w-64 shrink-0">
                    <input
                      type="text"
                      value={selectedDeliverableSpecialist ? deliverableAssigneeDisplay : deliverableAssignableSearch}
                      onChange={(e) => {
                        setDeliverableAssignableSearch(e.target.value);
                        setDeliverableAssignToOpen(true);
                        setTemplateAssignToOpen(false);
                        if (selectedDeliverableSpecialist && e.target.value !== deliverableAssigneeDisplay) {
                          setSelectedDeliverableSpecialist("");
                          setDeliverableAssigneeDisplay("");
                        }
                      }}
                      onFocus={() => {
                        setDeliverableAssignToOpen(true);
                        setTemplateAssignToOpen(false);
                      }}
                      placeholder="Assigned selector"
                      className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                    {deliverableAssignToOpen && (
                      <ul className="absolute z-50 mt-1 w-full max-h-48 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg py-1">
                        {deliverableAssignableLoading ? (
                          <li className="px-3 py-2 text-sm text-gray-500">Loading…</li>
                        ) : deliverableAssignableUsers.length === 0 ? (
                          <li className="px-3 py-2 text-sm text-gray-500">No users found. Try a different search.</li>
                        ) : (
                          deliverableAssignableUsers.map((u) => (
                            <li key={u.id}>
                              <button
                                type="button"
                                className="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 flex items-center justify-between"
                                onClick={() => {
                                  setSelectedDeliverableSpecialist(u.id);
                                  setDeliverableAssigneeDisplay(u.name || u.email);
                                  setDeliverableAssignableSearch("");
                                  setDeliverableAssignToOpen(false);
                                }}
                              >
                                <span>{u.name || u.email}</span>
                                {u.role && <span className="text-xs text-gray-500 ml-2">{u.role.replace("_", " ")}</span>}
                              </button>
                            </li>
                          ))
                        )}
                      </ul>
                    )}
                  </div>
                  <button
                    type="button"
                    onClick={clearDeliverableTaskSelection}
                    disabled={selectedDeliverableTaskIds.length === 0}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Clear selection
                  </button>
                </div>
              </div>
              <div className="space-y-3">
                {deliverablesPreviewTasks.map((task, index) => (
                  <div key={task.tempId} className="p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-start">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={selectedDeliverableTaskIds.includes(task.tempId)}
                            onChange={(e) => toggleDeliverableTaskSelection(task.tempId, e.target.checked)}
                            className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-2 focus:ring-primary-500"
                          />
                          <span className="text-sm font-medium text-gray-500">#{index + 1}</span>
                          <h4 className="font-medium text-gray-900">{task.title}</h4>
                          <span className="px-2 py-1 text-xs font-medium rounded-full flex items-center space-x-1 bg-yellow-100 text-yellow-700">
                            {getPriorityIcon("medium")}
                            <span className="capitalize">medium</span>
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 mt-1">{task.description}</p>
                        <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                          <span className="px-2 py-1 bg-gray-200 rounded">{task.category}</span>
                          <span className="px-2 py-1 bg-primary-100 text-primary-700 rounded">
                            Due: {task.dueDate}
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              </>
              )}
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-100/80 rounded-b-2xl flex-shrink-0">
          <button
            onClick={() => setOpen(false)}
            className="px-4 py-2.5 border border-gray-300 bg-white text-gray-700 rounded-xl hover:bg-gray-50 font-medium transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={handleCreateTasks}
            disabled={loading || !selectedTemplate || !selectedClient}
            className="px-6 py-2.5 rounded-xl font-semibold text-white bg-gradient-to-r from-primary-600 to-blue-600 hover:from-primary-700 hover:to-blue-700 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center space-x-2"
          >
            {loading ? (
              <>
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                <span>Creating Tasks...</span>
              </>
            ) : (
              <>
                <Plus className="h-4 w-4" />
                <span>
                  Create{" "}
                  {(selectedTemplate?.tasks.length || 0) + (autoGenerateDeliverables ? 12 : 0)}{" "}
                  Tasks
                </span>
              </>
            )}
          </button>
        </div>
      </div>
    </div>
  );
};

export default OnboardingTemplateModal;
